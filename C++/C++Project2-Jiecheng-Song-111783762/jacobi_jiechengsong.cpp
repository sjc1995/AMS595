...
int main(){
  ...
  double result[a][b] = {0.0}, original[a][b] = {0.0};
  clock_t start, end;
  double total_time;
  ...
  result be the matrix representing the mesh, produce an orginal matrix
  to store the original data, so we can use the original data and jacobi
  method, start, end and total_time will be used to calculate time.*/
  start = clock();
  for(i=0;i<a;i++){
    result[i][b-1] = 1.0;
  }
  ...
  if(N > 0){
	...
	for(j=1;j<b;j++){
      ...
      /*copy the array to array called original*/
      for(i=1;i<a-1;i++){
	for(j=1;j<b-1;j++){
	  old = result[i][j];
	  result[i][j] = (original[i-1][j]+original[i+1][j]+
			  original[i][j-1]+original[i][j+1])/4;	  
    ...
    and we use jacobi method to calculate the PDE, because I found if we input
    a string N will be a 0, so we use N > 0 as a condition*/
  }else{
    while(diff>=precise){
      diff = 0.0;
      k = k + 1;
